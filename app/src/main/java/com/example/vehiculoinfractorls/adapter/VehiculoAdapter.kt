...
class VehiculoAdapter(
    ...
    inner class VehiculoViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val tvInfraccion: TextView = itemView.findViewById(R.id.tvInfraccion)
    ...
    override fun onBindViewHolder(holder: VehiculoViewHolder, position: Int) {
        ...
        val colorGravedad = when (vehiculo.gravedad) {
            "GRAVE" -> Color.parseColor("#D32F2F")
            "MODERADA" -> Color.parseColor("#F57C00")
            "LEVE" -> Color.parseColor("#FBC02D")
            else -> Color.parseColor("#388E3C")
        }

        holder.tvGravedad.setBackgroundColor(colorGravedad)
